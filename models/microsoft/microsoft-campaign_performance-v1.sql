...
FROM
  `uk-data-cli-superdrug.dpl_uk_data_cli_superdrug_bingads.campaign_performance_daily_report` A
LEFT JOIN (
  SELECT
    name account_name,
    id,
  ...
  FROM
    `uk-data-cli-superdrug.dpl_uk_data_cli_superdrug_bingads.account_history`) B
ON
  A.account_id=B.id
LEFT JOIN (
  SELECT
    ...
    type,
  FROM
    `uk-data-cli-superdrug.dpl_uk_data_cli_superdrug_bingads.campaign_history` ) C
ON
  A.campaign_id=C.id
LEFT JOIN (
  SELECT
  ...
  FROM
    `uk-data-cli-superdrug.dpl_uk_data_cli_superdrug_bingads.account_impression_performance_daily_report`
